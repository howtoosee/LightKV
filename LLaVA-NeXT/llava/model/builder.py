...
def load_kv_cache_optim_model(
    model_path, model_base, model_name, attn_implementation="sdpa", customized_config=None, overwrite_config=None, **kwargs,
):
    if "mistral" in model_name.lower():
        # TODO
        raise NotImplementedError("Mistral model is not supported yet.")

    ## LLAMA
    tokenizer = AutoTokenizer.from_pretrained(model_path)
    if "lightkv" in model_name.lower():
        from llava.model.language_model.llava_llama_lightkv import LlavaLlamaLightKVForCausalLM, LlavaConfig

        llava_cfg = LlavaConfig.from_pretrained(model_path)
        if "v1.5" in model_name.lower():
            llava_cfg.delay_load = True  # a workaround for correctly loading v1.5 models

        model = LlavaLlamaLightKVForCausalLM.from_pretrained(
            model_path, config=llava_cfg, low_cpu_mem_usage=True, attn_implementation=attn_implementation, **kwargs
        )
    else:
        raise NotImplementedError(f"LLaMA model `{model_name}` is not supported yet.")

    return model, tokenizer


def load_pretrained_model(
    model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", torch_dtype="float16", attn_implementation="sdpa",
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                ...
                model = LlavaMixtralForCausalLM.from_pretrained(
                    model_base, low_cpu_mem_usage=True, config=lora_cfg_pretrained, attn_implementation=attn_implementation, **kwargs
                    )
            ...
            if "mixtral" in model_name.lower():
                ...
                model = LlavaMixtralForCausalLM.from_pretrained(
                    model_base, low_cpu_mem_usage=True, config=cfg_pretrained, attn_implementation=attn_implementation, **kwargs
                    )
            ...
            rank0_print(f"Loaded LLaVA model {model_name} of path {model_path}")
            if "lightkv" in model_name.lower():
                model, tokenizer = load_kv_cache_optim_model(
                    model_path, model_base, model_name, attn_implementation, customized_config, overwrite_config, **kwargs
                )
                ...
                tokenizer = AutoTokenizer.from_pretrained(model_path)
                model = LlavaMixtralForCausalLM.from_pretrained(
                    model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, config=llava_cfg, **kwargs
                    )
                ...
                cfg_pretrained = AutoConfig.from_pretrained(model_path)
                model = LlavaGemmaForCausalLM.from_pretrained(
                    model_path, low_cpu_mem_usage=True, config=cfg_pretrained, attn_implementation=attn_implementation, **kwargs
                    )
            else:
                try:
                    from llava.model.language_model.llava_llama import LlavaConfig
